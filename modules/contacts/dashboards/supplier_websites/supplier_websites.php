...
// +-----------------------------------------------------------------+
//  Path: /modules/contacts/dashboards/supplier_websites/supplier_websites.php
//
// Revision history
// 2011-07-01 - Added version number for revision control
define('DASHBOARD_SUPPLIER_WEBSITES_VERSION','3.2');

class supplier_websites extends ctl_panel {
  ...
  function Output($params) {
	...
	if ($result->RecordCount() < 1) {
	  $contents = CP_SUPPLIER_WEBSITES_NO_RESULTS;
	} else {
	  while (!$result->EOF) {
	...
	return $this->build_div(CP_SUPPLIER_WEBSITES_TITLE, $contents, $control);
  }

  function Update() {
	global $db;
	$my_title  = db_prepare_input($_POST['supplier_websites_field_0']);
	$my_url    = db_prepare_input($_POST['supplier_websites_field_1']);
