...
class inventory {
	...
	public function __construct($id, $sku, $inactive, $inventory_type, $description_short, $description_purchase, $description_sales, $image_with_path, $account_sales_income, $account_inventory_wage, $account_cost_of_sales, $item_taxable, $purch_taxable, $item_cost, $cost_method, $price_sheet, $price_sheet_v, $full_price, $item_weight, $quantity_on_hand, $quantity_on_order, $quantity_on_sales_order, $quantity_on_allocation, $minimum_stock_level, $reorder_quantity, $supplier_id, $lead_time, $upc_code, $serialize, $creation_date, $last_update, $last_journal_date){
		$product['id']  					= $id ;
		$product['sku'] 					= $sku ;
	...
	function create_new() {
		...
		gen_add_audit_log(INV_LOG_INVENTORY . TEXT_ADD, TEXT_TYPE . ': ' . $product['inventory_type'] . ' - ' . $product['sku'] );
	}
	...
	function check_remove() {
		...
		$result = $db->Execute("select id from " . TABLE_INVENTORY_HISTORY . " where sku = '" . $product['sku'] . "' and remaining > 0");
		if ($result->RecordCount() > 0) {
		 	$messageStack->add(INV_ERROR_DELETE_HISTORY_EXISTS, 'error');
		 	return false;
		}
		$result = $db->Execute( "select id from " . TABLE_JOURNAL_ITEM . " where sku = '" . $product['sku'] . "' limit 1");
		if ($result->Recordcount() > 0) {
			$messageStack->add('there is a journal item that uses this sku', 'error');
	  		return false;	
		}
		$this->remove();
	  	return true;
	...
	function remove(){
		$db->Execute("delete from " . TABLE_INVENTORY . " where id = " . $product['id']);
	  	if ($product['image_with_path']) { // delete image
			$file_path = DIR_FS_MY_FILES . $_SESSION['company'] . '/inventory/images/';
			if (file_exists($file_path . $product['image_with_path'])) unlink ($file_path . $product['image_with_path']);
	  	}
	  	$db->Execute("delete from " . TABLE_INVENTORY_SPECIAL_PRICES . " where inventory_id = '" . $product['id'] . "'");
		gen_add_audit_log(INV_LOG_INVENTORY . TEXT_DELETE, $product['sku']);
	}
